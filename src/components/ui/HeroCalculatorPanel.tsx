...
import { emiCalculator } from '@/calculators/loan-emi/emi-calculator';
...
// Static placeholder data for tabs without a compute() function yet
type StaticDisplay = {
  title: string;
  slug: string;
...
const STATIC_DISPLAYS: Partial<Record<TabId, StaticDisplay>> = {
  sip: {
...
// EMI tab defaults (compact hero amounts, not the full calculator defaults)
const EMI_DEFAULTS = { loanAmount: 2500000, interestRate: 8.5, tenureYears: 20 };

function HeroEmiTab({ currency }: { currency: string }) {
  const [loanAmount, setLoanAmount] = useState(EMI_DEFAULTS.loanAmount);
  const [interestRate, setInterestRate] = useState(EMI_DEFAULTS.interestRate);
  const [tenureYears, setTenureYears] = useState(EMI_DEFAULTS.tenureYears);
  const router = useRouter();

  const results = emiCalculator.compute({ loanAmount, interestRate, tenureYears });
  const emi = results.emi;

  const loanPct = Math.round(((loanAmount - 500000) / (10000000 - 500000)) * 100);
  const ratePct = Math.round(((interestRate - 6) / (24 - 6)) * 100);
  const tenurePct = Math.round(((tenureYears - 1) / (30 - 1)) * 100);

  const sliderStyle = (pct: number) => ({
    background: `linear-gradient(to right, #1B4FD8 ${pct}%, #EEF2FF ${pct}%)`,
  });
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '18px' }}>
        <span style={{ fontSize: '14px', fontWeight: 600, color: '#111827' }}>EMI Calculator</span>
        <button
          onClick={() => router.push('/calculators/emi-calculator/')}
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '6px' }}>
            <span style={{ fontSize: '11px', color: '#6B7280' }}>Loan amount</span>
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '6px' }}>
            <span style={{ fontSize: '11px', color: '#6B7280' }}>Interest rate (% p.a.)</span>
            <span style={{ fontSize: '13px', fontWeight: 600, color: '#111827' }}>
              {formatPercent(interestRate)}
            </span>
          </div>
          <input
            type="range" min={6} max={24} step={0.25}
            value={interestRate}
            onChange={(e) => setInterestRate(Number(e.target.value))}
            className="hero-slider"
            style={sliderStyle(ratePct)}
        ...
        <div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '6px' }}>
            <span style={{ fontSize: '11px', color: '#6B7280' }}>Loan tenure</span>
            <span style={{ fontSize: '13px', fontWeight: 600, color: '#111827' }}>
      ...
      <div style={{
        ...
        <span style={{ fontSize: '12.5px', color: '#374151', fontWeight: 500 }}>Monthly EMI</span>
      ...
      <button
        ...
        style={{
          ...
          background: '#1B4FD8', color: '#ffffff', fontSize: '13px',
      ...
      <p style={{ margin: '8px 0 0', fontSize: '10.5px', color: '#9CA3AF', textAlign: 'center' }}>
        Opens with amortization schedule, charts and PDF export
      </p>
    </div>
  );
}

function StaticTab({ data, onNavigate }: { data: StaticDisplay; onNavigate: () => void }) {
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '18px' }}>
        <span style={{ fontSize: '14px', fontWeight: 600, color: '#111827' }}>{data.title}</span>
        <button
          onClick={onNavigate}
          style={{ fontSize: '12px', color: '#6B7280', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}
        >
        ...
        {data.inputs.map((input) => (
          <div key={input.label}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '6px' }}>
              ...
              {input.hint && <span style={{ fontSize: '10.5px', color: '#9CA3AF' }}>{input.hint}</span>}
            </div>
      ...
      <div style={{
        ...
        <span style={{ fontSize: '12.5px', color: '#374151', fontWeight: 500 }}>{data.resultLabel}</span>
        <span style={{ fontSize: '17px', fontWeight: 700, color: '#1B4FD8' }}>{data.result}</span>
      </div>
...
export function HeroCalculatorPanel() {
  ...
  const staticData = STATIC_DISPLAYS[activeTab];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      ...
      <div style={{
        ...
        border: '0.5px solid #E5E7EB', padding: '20px 22px 22px',
        boxShadow: '0 1px 3px rgba(0,0,0,0.06), 0 4px 16px rgba(0,0,0,0.06)',
      }}>
        {activeTab === 'emi' ? (
          <HeroEmiTab currency={currency} />
        ) : staticData ? (
          <StaticTab
            data={staticData}
            onNavigate={() => router.push(`/calculators/${staticData.slug}/`)}
          />
        ) : null}
      </div>
    </div>
